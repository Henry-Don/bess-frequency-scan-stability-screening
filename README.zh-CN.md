...
准备发布的版本为 **v0.6.1**。验证范围包含 0.5 Hz 至 100 Hz 的 20 个对数间隔频点、四档 SCR、四档 PLL 增益缩放以及独立的 d 轴/q 轴注入。完整 4 x 4 运行矩阵对应 640 次注入仿真；全部数值阈值均在矩阵比较前固定。
...
持续验证采用分层设计：`.github/workflows/ci.yml` 在每次推送和拉取请求时运行仓库检查及真实模型快速回归；`.github/workflows/full-regression.yml` 提供手动触发的完整七组、640 次注入回归，时间上限为 360 分钟。仓库检查入口为 `python/verify_repository.py`，用于确认公开文件结构并排除生成的 Simulink 缓存文件。
...
完整本地入口显示 7/7 组检查通过。独立 10 Hz 重复性检查测得幅值漂移 0%、相位漂移 0 度；九路基准信号均满足设定的数值等效容差。验证版本中的 43 个 MATLAB 文件静态检查为零项问题。
