...
% Fixed data-quality limits for the 10 Hz reference identification.  The
% near-zero cross-voltage channels use leakage ratios instead of their own
% relative residuals, which would otherwise be dominated by a tiny scale.
scan_cfg.fit_quality_schema_version = 1;
scan_cfg.fit_quality_thresholds = struct( ...
    'main_voltage_relative_residual', 0.02, ...
    'current_matrix_weighted_residual', 0.35, ...
    'reference_current_relative_residual', 0.35, ...
    'cross_voltage_leakage_ratio', 0.05);
scan_cfg.repeatability_thresholds = struct( ...
    'admittance_magnitude_relative_difference', 0.02, ...
    'admittance_phase_difference_deg', 2.0);
scan_cfg.baseline_equivalence_thresholds = struct( ...
    'relative_rms_error', 1e-8, ...
    'normalized_maximum_error', 1e-8);
